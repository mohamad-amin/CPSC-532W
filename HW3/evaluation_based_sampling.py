...
def compute_expectation_IS(data, f, *args):
    rs, logWs = data
    ws = np.exp(np.array(logWs))
    return (ws * f(rs, *args)).sum() / ws.sum()
...
def extract_asts():
    ...
    for i in range(1, 6):
        ast = daphne(['desugar', '-i', '../HW2/programs/tests/hw3/program_{}.daphne'.format(i)])
        with open('programs/tests/hw3/program_{}_ast.pkl'.format(i), 'wb') as f:
            pkl.dump(ast, f)
...
def load_ast_env(i):
    return pkl.load(open('programs/tests/hw3/program_{}_ast.pkl'.format(i), 'rb'))
...
# os.chdir('HW2')
# from HW2.evaluation_based_sampling import *
